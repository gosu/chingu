...
module Chingu
  ...
  class Rect < Array
    ...
    def initialize(*argv)
      case argv.length
      when 1
        if argv[0].kind_of? Array
          super(argv[0])
        elsif argv[0].respond_to? :rect
          super(argv[0])
        end
      when 2
    ...
    def Rect.new_from_object(object)
      ...
      when Array
        if object.length >= 4
          ...
          raise(ArgumentError,
                "Array does not have enough indices to be made into a Rect (%d for 4)."%object.length)
        end
      else
        begin
          case(object.rect)
          ...
          when Array
            if object.rect.length >= 4
              ...
              raise(ArgumentError,
                    "Array does not have enough indices to be made into a Rect (%d for 4)."%object.rect.length)
            end
          end
        rescue NoMethodError # if no rect.rect
          raise(TypeError,
                "Object must be a Rect or Array [x,y,w,h], or have an attribute called 'rect'. (Got %s instance.)"%object.class)
        end
      end
    end
    ...
    def inspect
      "#<Rect:#{self.object_id} [%s,%s,%s,%s]>"%self
    end
    ...
    def size=(size)
     raise ArgumentError,
           "Rect#size= takes an Array of form [width, height]." if size.size != 2
     self[2,2] = size
    ...
    def center=(center)
      raise ArgumentError,
      "Rect#center= takes an Array of the form [x,y]." if center.size != 2
    ...
    def centerx
      return self.at(0) + (self.at(2).div(2))
    end
    ...
    def centery
      return self.at(1) + (self.at(3).div(2))
    end
    ...
    def topleft=(topleft)
      raise ArgumentError,
            "Rect#topright= takes an Array of form [x, y]." if topleft.size != 2
    ...
    def topright=(topright)
      raise ArgumentError,
      "Rect#topright= takes an Array of form [x, y]." if topright.size != 2
    ...
    def bottomleft=(bottomleft)
      raise ArgumentError,
            "Rect#bottomleft= takes an Array of form [x, y]." if bottomleft.size != 2
    ...
    def bottomright=(bottomright)
      raise ArgumentError,
            "Rect#bottomright= takes an Array of form [x, y]." if bottomright.size != 2
    ...
    def midleft=(midleft)
      raise ArgumentError,
            "Rect#midleft= takes an Array of form [x, y]." if midleft.size != 2
    ...
    def midtop=(midtop)
      raise ArgumentError,
            "Rect#midtop= takes an Array of form [x, y]." if midtop.size != 2
    ...
    def midright=(midright)
      raise ArgumentError,
            "Rect#midright= takes an Array of form [x, y]." if midright.size != 2
    ...
    def midbottom=(midbottom)
      raise ArgumentError,
            "Rect#midbottom= takes an Array of form [x, y]." if midbottom.size != 2
    ...
    def collide_hash(hash_rects)
      hash_rects.each do |key, value|
        return [key, value] if value.collide_rect? + (self)
      end
    ...
    def collide_hash_all(hash_rects)
      hash_rects.select do |key, value|
